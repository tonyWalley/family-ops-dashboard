  ...
  for(const file of files){
    try{
      ...
      for(const row of parsed.records){
        try{const t=normalizeRecord(source,account,row),base=fingerprintBase(t),occ=(occurrences[base]=(occurrences[base]||0)+1),fingerprint=`${source.toLowerCase()}_${base}_${occ}`;t.fingerprint=fingerprint;if(existing.has(fingerprint)){fileDup++;continue}existing.add(fingerprint);normalized.push(t)}catch(e){fileInvalid++}
      }
